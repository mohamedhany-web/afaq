{{-- CRM — إدارة عليا (داخل قسم المبيعات) --}}
...
<div class="mt-6">
    ...
    @endcanNav
    @can('viewDeletionLog', \App\Models\Client::class)
    <a href="{{ route('crm.clients.deletions.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('crm.clients.deletions.*') ? 'active' : '' }}">
    ...
    @if(auth()->user()?->canAccessOperations() || auth()->user()?->hasRole(['super_admin', 'admin']))
    ...
    @endcanNav
    @endcanNav
    @canNav('manage-sales-teams', 'view-team-sales')
    <a href="{{ route('crm.teams.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('crm.teams.*') ? 'active' : '' }}">
...
<div class="mt-6">
    ...
    <a href="{{ route('operations.follow-ups.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium">
        <svg class="ml-3 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
        متابعات العمليات
    </a>
    @endif
...
<div class="mt-6">
    ...
    @endcanNav
    @canNav('edit-settings')
    <a href="{{ route('admin.auto-penalties.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('admin.auto-penalties.*') ? 'active' : '' }}">
        <svg class="ml-3 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        قواعد الخصومات
    </a>
    @endcanNav
    @include('layouts.partials.sidebar-leaves-link', ['label' => 'الإجازات'])
</div>
<div class="mt-6">
    <h3 class="sidebar-section-title px-4">الوكلاء والعمولات</h3>
    @canNav('view-sales')
    <a href="{{ route('crm.freelance-agents.index') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('crm.freelance-agents.*') ? 'active' : '' }}">
        <svg class="ml-3 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
        الوكلاء المستقلون
    </a>
    <a href="{{ route('crm.freelance-agents.scheme') }}" class="sidebar-link flex items-center px-4 py-3 text-sm font-medium {{ request()->routeIs('crm.freelance-agents.scheme') ? 'active' : '' }}">
        <svg class="ml-3 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-7M7 3h10a2 2 0 012 2v14a2 2 0 01-2 2H7a2 2 0 01-2-2V5a2 2 0 012-2z"/></svg>
        جدول هيكل العمولات
    </a>
    @endcanNav
