@extends('layouts.app')
@section('page-title', 'المطورون العقاريون')
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
    @include('crm.partials.stat-card', ['label' => 'إجمالي المطورين', 'value' => $stats['total'], 'accent' => 'theme', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />'])
    @include('crm.partials.stat-card', ['label' => 'تعاقدات نشطة', 'value' => $stats['contracted'], 'accent' => 'green', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />'])
    @include('crm.partials.stat-card', ['label' => 'بوابات مفعّلة', 'value' => $stats['portal_ready'], 'accent' => 'purple', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />'])
    @include('crm.partials.stat-card', ['label' => 'المشاريع المرتبطة', 'value' => $stats['projects'], 'accent' => 'blue', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5" />'])
</div>
...
<div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-4 sm:p-5 mb-6">
    <form method="GET" class="flex flex-col lg:flex-row gap-3 lg:items-end">
        <div class="flex-1">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="الاسم، البريد، الهاتف، أو المدينة..."
        ...
        <div class="w-full lg:w-40">
            ...
            <select name="status" class="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 font-tajawal text-sm">
                ...
                @foreach($statusLabels as $val => $txt)
        ...
        <div class="w-full lg:w-44">
            ...
            <select name="contract" class="w-full border-2 border-gray-200 rounded-xl px-4 py-2.5 font-tajawal text-sm">
                <option value="">الكل</option>
                <option value="active" @selected(request('contract') === 'active')>تعاقد نشط</option>
                <option value="none" @selected(request('contract') === 'none')>بدون تعاقد</option>
            </select>
        </div>
        <div class="flex gap-2">
            ...
            @if(request()->hasAny(['search', 'status', 'contract']))
            <a href="{{ route('admin.developers.index') }}" class="px-5 py-2.5 rounded-xl border-2 border-gray-200 text-gray-600 text-sm font-semibold hover:bg-gray-50 font-tajawal">مسح</a>
            @endif
        </div>
    </form>
</div>
...
<div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
    <div class="px-5 sm:px-6 py-4 border-b border-gray-200 flex items-center justify-between" style="{{ $headerStyle }}">
        ...
        <span class="text-xs px-3 py-1 rounded-full font-medium font-tajawal" style="background: {{ $themeColor }}15; color: {{ $themeColor }};">{{ $developers->total() }} مطور</span>
    </div>
                ...
                <tr class="hover:bg-gray-50/80 transition-colors">
                    <td class="p-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <a href="{{ route('admin.developers.show', $d) }}" class="font-semibold text-gray-900 hover:underline font-tajawal block truncate">{{ $d->name }}</a>
                                @if($d->city)<span class="text-xs text-gray-400 font-tajawal">{{ $d->city }}</span>@endif
                            </div>
                        </div>
                    </td>
                    <td class="p-4">
                        <div class="text-xs text-gray-600 font-tajawal" dir="ltr">{{ $d->phone ?: '—' }}</div>
                        <div class="text-xs text-gray-400 font-tajawal truncate max-w-[180px]" dir="ltr">{{ $d->email ?: '—' }}</div>
                    </td>
                    <td class="p-4">
                        @if($d->activeContract)
                        <span class="text-xs px-2 py-1 rounded-full font-semibold font-tajawal bg-emerald-100 text-emerald-800">نشط</span>
                        @if($d->activeContract->commission_percent)
                        <div class="text-xs text-gray-500 mt-1 font-tajawal tabular-nums">{{ $d->activeContract->commission_percent }}% عمولة</div>
                        @endif
                        @else
                        <span class="text-xs px-2 py-1 rounded-full font-semibold font-tajawal bg-amber-100 text-amber-800">بدون تعاقد</span>
                        @endif
                    </td>
                    <td class="p-4 font-semibold text-gray-900 font-tajawal tabular-nums">{{ $d->projects_count }}</td>
                    <td class="p-4">
                        @if($d->isPortalReady())
                        <span class="text-xs px-2 py-1 rounded-full font-semibold font-tajawal bg-green-100 text-green-800">مفعّلة</span>
                        @elseif($d->portal_enabled)
                    ...
                    <td class="p-4">
                        <span class="text-xs px-2 py-1 rounded-full font-semibold font-tajawal {{ $stClass }}">{{ $st }}</span>
                    </td>
                    <td class="p-4">
                        <div class="flex flex-wrap gap-1.5">
                            <a href="{{ route('admin.developers.show', $d) }}"
                               class="px-2.5 py-1.5 rounded-lg text-xs font-semibold text-white font-tajawal"
                               style="background:{{ $themeColor }}">عرض</a>
                            @if($canManage)
                            <a href="{{ route('admin.developers.edit', $d) }}"
                ...
                <tr>
                    <td colspan="7" class="p-12 text-center">
                        ...
                        <a href="{{ route('admin.developers.create') }}"
                           ...
                           style="background: linear-gradient(135deg, {{ $themeColor }} 0%, {{ $themeColor }}dd 100%);">إضافة أول مطور</a>
                        @endif
                    </td>
                </tr>
                @endforelse
    ...
    <div class="p-4 border-t border-gray-100 hidden md:block">{{ $developers->links() }}</div>
    @endif
</div>
...
<div class="md:hidden mt-4 space-y-3">
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-4 shadow-sm">
        <div class="flex items-start gap-3">
            <div class="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0 font-tajawal"
                 style="background: linear-gradient(135deg, {{ $themeColor }} 0%, {{ $themeColor }}dd 100%);">{{ mb_substr($d->name, 0, 1) }}</div>
            <div class="flex-1 min-w-0">
                <a href="{{ route('admin.developers.show', $d) }}" class="font-bold text-gray-900 font-tajawal block truncate">{{ $d->name }}</a>
                <p class="text-xs text-gray-500 mt-1 font-tajawal">{{ $d->city ?: '—' }} · {{ $d->projects_count }} مشروع</p>
            </div>
        </div>
        <div class="flex gap-2 mt-3">
            <a href="{{ route('admin.developers.show', $d) }}" class="flex-1 text-center py-2 rounded-lg text-xs font-bold text-white font-tajawal" style="background:{{ $themeColor }}">عرض</a>
            @if($canManage)
            <a href="{{ route('admin.developers.edit', $d) }}" class="px-4 py-2 rounded-lg text-xs font-semibold border border-gray-200 font-tajawal">تعديل</a>
            @endif
        </div>
    </div>
    @empty
    ...
    @endforelse
    @if($developers->hasPages())
    <div class="pt-2">{{ $developers->links() }}</div>
    @endif
</div>
@endsection
