@extends('layouts.app')
@section('page-title', 'فريق مبيعات جديد')
...
<form action="{{ route('crm.teams.store') }}" method="POST" class="w-full max-w-4xl mx-auto space-y-6">
    @csrf
    @include('crm.teams.partials.form')
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <a href="{{ ($isScopedManager ?? false) ? route('crm.dashboard') : route('crm.teams.index') }}" class="inline-flex items-center justify-center px-6 py-3 rounded-xl border-2 border-gray-200 text-gray-600 font-semibold text-sm hover:bg-gray-50 font-tajawal">
            إلغاء
        </a>
