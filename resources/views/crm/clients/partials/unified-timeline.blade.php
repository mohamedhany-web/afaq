...
<div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-6 w-full">
    <div class="px-5 sm:px-6 py-4 border-b border-gray-200 flex items-center justify-between"
        ...
        <span class="text-xs px-3 py-1 rounded-full font-medium font-tajawal bg-gray-100 text-gray-600">{{ $timeline->count() }} حدث</span>
    </div>
        ...
        @forelse($timeline as $event)
            ...
            <div class="flex gap-4 pb-6 last:pb-0 relative">
                <div class="flex flex-col items-center">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0" style="background: {{ $deptColor }}15;">
                        <svg class="w-5 h-5" style="color: {{ $deptColor }};" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex-1 min-w-0 pb-2">
                    <div class="flex flex-wrap items-center gap-2 mb-1">
                        <span class="font-bold text-sm text-gray-900 font-tajawal">{{ $title }}</span>
                        <span class="text-[10px] px-2 py-0.5 rounded-full font-semibold" style="background: {{ $deptColor }}12; color: {{ $deptColor }};">{{ $deptLabel }}</span>
                    </div>
                    @if($desc)
                        <p class="text-sm text-gray-600 font-tajawal whitespace-pre-line">{{ Str::limit($desc, 200) }}</p>
                    @endif
                    @if(!empty($meta['lost_reason_label']))
                        <p class="text-xs text-red-600 mt-1 font-tajawal">سبب الخسارة: {{ $meta['lost_reason_label'] }}</p>
                    @endif
                    <div class="flex flex-wrap gap-3 mt-2 text-xs text-gray-400 font-tajawal">
                        <span>{{ $occurred instanceof \Carbon\Carbon ? $occurred->format('Y/m/d H:i') : \Carbon\Carbon::parse($occurred)->format('Y/m/d H:i') }}</span>
                        @if($user)
                            <span>{{ $user->name ?? '—' }}</span>
                        @endif
                    </div>
