...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
    @include('crm.partials.stat-card', ['label' => 'الصفقات', 'value' => $dealsCount, 'accent' => 'theme', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7" />'])
    ...
    @include('crm.partials.stat-card', ['label' => 'الحالة', 'value' => match($client->status) { 'prospect' => 'محتمل', 'active' => 'نشط', 'inactive' => 'غير نشط', 'suspended' => 'موقوف', default => $client->status }, 'accent' => 'blue', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />'])
    @include('crm.partials.stat-card', ['label' => 'تاريخ التسجيل', 'value' => $client->created_at->format('Y/m/d'), 'accent' => 'purple', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />'])
</div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        ...
        <div class="p-5 sm:p-6 space-y-4">
            ...
            <div>
                ...
                <dd class="text-gray-900 font-tajawal">{{ trim($client->assignedEmployee->first_name . ' ' . $client->assignedEmployee->last_name) }}</dd>
            </div>
            @endif
            <div>
                ...
                <a href="{{ route('crm.pipeline.show', $sale) }}" class="block p-4 mb-3 last:mb-0 rounded-xl border border-gray-100 hover:border-gray-200 hover:bg-gray-50/80 transition-all">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <div class="min-w-0">
                            ...
                            @if($sale->project)
                                <div class="text-xs text-gray-500 mt-1 font-tajawal">{{ $sale->project->name }}</div>
                            @endif
                        </div>
