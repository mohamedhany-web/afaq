...
<div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-6">
    ...
    <form action="{{ route('crm.daily-reports.generate') }}" method="POST" class="p-5 sm:p-6 flex flex-col sm:flex-row gap-4 items-end">
        @csrf
        <div class="flex-1 w-full">
            <label class="block text-xs font-bold text-gray-500 mb-1.5 font-tajawal">تاريخ التقرير</label>
            <input type="date" name="report_date" value="{{ old('report_date', today()->toDateString()) }}" max="{{ today()->toDateString() }}" required
            ...
            @error('report_date')<p class="mt-1 text-sm text-red-600 font-tajawal">{{ $message }}</p>@enderror
        </div>
