...
<div class="flex flex-wrap items-center gap-2 mb-4">
    ...
    <a href="{{ route('crm.pipeline.index', $query) }}"
       class="px-3 py-1.5 rounded-lg text-xs font-semibold font-tajawal {{ ($current ?? 'kanban') === 'kanban' ? 'text-white' : 'border border-gray-200 text-gray-700 hover:bg-gray-50' }}"
       @if(($current ?? 'kanban') === 'kanban') style="background: {{ $themeColor }};" @endif>Kanban — مسار العملاء</a>
    <a href="{{ route('crm.pipeline.index', array_merge($query, ['view' => 'list'])) }}"
       ...
       @if(($current ?? '') === 'list') style="background: {{ $themeColor }};" @endif>قائمة العملاء</a>
