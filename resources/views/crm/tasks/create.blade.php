...
<form action="{{ route('crm.tasks.store') }}" method="POST"
      ...
      x-data="{
          ...
          priorityColors: @js($priorityColors),
          priorityLabels: @js(config('crm_tasks.priority_labels', [])),
      }">
    @csrf

    <div class="grid grid-cols-1 xl:grid-cols-12 gap-4 sm:gap-6 items-start">
        <div class="xl:col-span-8 space-y-6">
            @include('crm.tasks.partials.form')
        </div>

        <aside class="xl:col-span-4 space-y-4 xl:sticky xl:top-24">
            <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                <div class="px-5 py-4 border-b" style="background: linear-gradient(135deg, {{ $themeColor }}12 0%, {{ $themeColor }}04 100%);">
                    <h3 class="font-bold text-gray-900">معاينة الأولوية</h3>
                </div>
                <div class="p-5">
                    <div class="h-2 rounded-full mb-3 transition-colors"
                         :style="'background:' + (priorityColors[priority] || '{{ $themeColor }}')"></div>
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-xs text-amber-900">
                <strong>تنبيه:</strong> الحد الأقصى {{ config('crm_tasks.max_open_tasks_per_user', 25) }} مهمة نشطة لكل موظف. عند التجاوز سيُرفض التعيين.
            </div>
        </aside>
    </div>
