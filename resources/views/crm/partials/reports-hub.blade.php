...
<div class="mb-6 bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden font-tajawal">
    <div class="px-5 sm:px-6 py-4 border-b border-gray-200 flex items-center justify-between gap-3"
        ...
        <a href="{{ route('crm.pipeline.index', ['view' => 'deals']) }}" class="text-xs font-semibold shrink-0 hover:underline" style="color: {{ $themeColor }}">مسار المبيعات</a>
    </div>
    <div class="p-4 sm:p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        @foreach($links as $link)
        <a href="{{ $link['url'] }}" class="flex items-start gap-3 p-4 rounded-xl border border-gray-100 hover:border-gray-200 hover:bg-gray-50/80 transition-all group">
            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style="background: {{ $themeColor }}12;">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="color: {{ $themeColor }}">{!! $link['icon'] !!}</svg>
            </span>
            <span class="min-w-0">
                <span class="block text-sm font-bold text-gray-900 group-hover:underline">{{ $link['label'] }}</span>
                <span class="block text-xs text-gray-500 mt-0.5">{{ $link['desc'] }}</span>
            </span>
        </a>
        @endforeach
    </div>
