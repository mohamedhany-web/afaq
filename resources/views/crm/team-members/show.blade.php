...
<div class="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mb-6">
    @include('crm.partials.stat-card', ['label' => 'إجمالي الصفقات', 'value' => $stats['total_deals'], 'accent' => 'theme', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7" />'])
    @include('crm.partials.stat-card', ['label' => 'صفقات نشطة', 'value' => $stats['active_deals'], 'accent' => 'blue', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />'])
    @include('crm.partials.stat-card', ['label' => 'تم البيع', 'value' => $stats['won_deals'], 'accent' => 'green', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />'])
    ...
    @include('crm.partials.stat-card', ['label' => 'تحديثات اليوم', 'value' => $stats['today_updates'], 'accent' => 'purple', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />'])
</div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        ...
        <div class="p-5 space-y-3 max-h-[400px] overflow-y-auto">
            @forelse($todayDeals as $deal)
                <a href="{{ route('crm.pipeline.show', $deal) }}" class="block p-3 rounded-xl border border-gray-100 hover:bg-gray-50">
                    <p class="font-semibold text-sm font-tajawal">{{ $deal->client?->name }} — {{ $deal->product_service }}</p>
                    <p class="text-xs text-gray-500 mt-1 font-tajawal">{{ $stageLabels[$deal->stage] ?? $deal->stage }} · {{ $money($deal->estimated_value) }} · {{ $deal->updated_at->format('H:i') }}</p>
                </a>
            @empty
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
        ...
        <div class="p-5 space-y-3 max-h-[400px] overflow-y-auto">
            @forelse($clients as $client)
                <a href="{{ route('crm.clients.show', $client) }}" class="block p-3 rounded-xl border border-gray-100 hover:bg-gray-50">
        ...
        <table class="w-full text-sm">
            ...
            @foreach($deals as $deal)
                <tr class="border-t border-gray-100 hover:bg-gray-50">
                    <td class="p-4"><a href="{{ route('crm.clients.show', $deal->client) }}" class="font-medium font-tajawal" style="color: {{ $themeColor }};">{{ $deal->client?->name }}</a></td>
