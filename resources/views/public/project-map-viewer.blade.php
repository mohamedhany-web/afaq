...
<head>
    ...
    <title>{{ $modeLabel }} — {{ $project->name }}</title>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .crm-map-pin-body { width:22px;height:22px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);border:2px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,.4);background:{{ $themeColor }}; }
    </style>
</head>
<body>
<header>
    <div>
        <h1>{{ $project->name }}</h1>
        <span>{{ $modeLabel }} · {{ $project->city ?? '' }}</span>
    </div>
...
@if($mode === 'streetview' && $streetEmbed)
    <iframe id="viewer-iframe" src="{{ $streetEmbed }}" allowfullscreen loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
@else
