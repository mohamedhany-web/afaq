...
class CrmProjectUnitController extends Controller
{
    ...
    public static function unitPayload(ProjectUnit $unit): array
    {
        ...
        return [
            ...
            'floor_label' => $unit->floor_label ?? $floor?->label,
            'apartment_number' => $unit->apartment_number,
            'direction' => $unit->direction,
            'direction_label' => $unit->directionLabel(),
            'unit_price_total' => $unit->unit_price_total ? (float) $unit->unit_price_total : null,
            ...
            'payment_plans' => $unit->paymentPlans->map(fn ($plan) => [
                ...
                'building_percent' => $plan->building_percent ? (float) $plan->building_percent : null,
                'discount_percent' => $plan->discount_percent ? (float) $plan->discount_percent : null,
                'loading_percent' => $plan->loading_percent ? (float) $plan->loading_percent : null,
                'net_unit_price' => $plan->net_unit_price ? (float) $plan->net_unit_price : null,
                'total_contract_amount' => $plan->total_contract_amount ? (float) $plan->total_contract_amount : null,
                'maintenance_deposit' => $plan->maintenance_deposit ? (float) $plan->maintenance_deposit : null,
                'remaining_balance' => $plan->remaining_balance ? (float) $plan->remaining_balance : null,
