...
<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6 font-tajawal ui-compact-hidden" id="crm-pulse-details">
    <?php $__currentLoopData = $cards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $card): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <?php $items = $crmPulse['recent'][$card['key']] ?? collect(); ?>
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        <div class="px-4 py-3 border-b flex items-center justify-between gap-2"
             style="background: linear-gradient(135deg, <?php echo e($themeColor); ?>06, transparent);">
            <div>
                <h3 class="text-sm font-bold text-gray-900"><?php echo e($card['label']); ?></h3>
                <?php if(!empty($card['hint'])): ?>
                <p class="text-[10px] text-gray-400"><?php echo e($card['hint']); ?></p>
                <?php endif; ?>
            </div>
            <a href="<?php echo e($card['href']); ?>" class="text-[10px] font-bold hover:underline shrink-0" style="color:<?php echo e($themeColor); ?>"><?php echo e($card['linkLabel']); ?></a>
        </div>
        <ul class="divide-y divide-gray-100 max-h-52 overflow-y-auto">
            <?php $__empty_1 = true; $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php if($card['key'] === 'today_comments' || $card['key'] === 'yesterday_comments'): ?>
                <li class="px-4 py-3 text-sm">
                    ...
                    <p class="text-xs text-gray-500 mt-0.5"><?php echo e($row->user?->name ?? '—'); ?> · <?php echo e($row->typeLabel()); ?></p>
                    <?php if($row->notes): ?>
                    <p class="text-xs text-gray-600 mt-1 line-clamp-2"><?php echo e($row->notes); ?></p>
                    <?php endif; ?>
                </li>
                <?php elseif($card['key'] === 'missed_reminders_yesterday'): ?>
                <li class="px-4 py-3 text-sm">
                    <?php if($row['type'] === 'follow_up'): ?>
                    <?php $fu = $row['item']; ?>
                    ...
                    <p class="text-xs text-gray-500">متابعة · <?php echo e($fu->user?->name); ?> — <?php echo e($fu->scheduled_at?->format('H:i')); ?> · <?php echo e($fu->typeLabel()); ?></p>
                    <?php else: ?>
                    <?php $task = $row['item']; ?>
                    ...
                    <p class="text-xs text-gray-500">مهمة · <?php echo e($task->assignee?->name); ?> — <?php echo e($task->due_at?->format('H:i')); ?></p>
                    <?php endif; ?>
                </li>
                <?php elseif($card['key'] === 'today_reminders'): ?>
                <li class="px-4 py-3 text-sm">
                    <?php if($row['type'] === 'follow_up'): ?>
                    <?php $fu = $row['item']; ?>
                    <a href="<?php echo e($fu->client?->profileUrl() ?? '#'); ?>" class="font-semibold hover:underline" style="color:<?php echo e($themeColor); ?>"><?php echo e($fu->client?->name ?? '—'); ?></a>
                    <p class="text-xs text-gray-500">متابعة · <?php echo e($fu->user?->name); ?> — <?php echo e($fu->scheduled_at?->format('H:i')); ?></p>
                    <?php else: ?>
                    <?php $task = $row['item']; ?>
                    <a href="<?php echo e(route('crm.tasks.show', $task)); ?>" class="font-semibold hover:underline" style="color:<?php echo e($themeColor); ?>"><?php echo e($task->title); ?></a>
                    <p class="text-xs text-gray-500">مهمة · <?php echo e($task->assignee?->name); ?> — <?php echo e($task->due_at?->format('H:i')); ?></p>
                ...
                <?php elseif($card['key'] === 'done_deals' || $card['key'] === 'cancelled_deals'): ?>
                <li class="px-4 py-3 text-sm">
                    <a href="<?php echo e(route('crm.pipeline.show', $row)); ?>" class="font-semibold hover:underline" style="color:<?php echo e($themeColor); ?>"><?php echo e($row->client?->name ?? $row->product_service); ?></a>
                    <p class="text-xs text-gray-500"><?php echo e($row->salesRep?->name ?? '—'); ?> · <?php echo e(number_format($row->estimated_value)); ?></p>
                </li>
                <?php elseif($card['key'] === 'potential_clients'): ?>
                <li class="px-4 py-3 text-sm">
                    <a href="<?php echo e($row->profileUrl()); ?>" class="font-semibold hover:underline" style="color:<?php echo e($themeColor); ?>"><?php echo e($row->name); ?></a>
                    <p class="text-xs text-gray-500">
                        <?php echo e($row->assignedEmployee ? trim($row->assignedEmployee->first_name.' '.$row->assignedEmployee->last_name) : 'غير مُعيَّن'); ?>

                        · <?php echo e($row->lead_stage ?? $row->status); ?>
...
<?php /**PATH C:\xampp\htdocs\afaq\resources\views\operations\partials\crm-pulse.blade.php ENDPATH**/ ?>