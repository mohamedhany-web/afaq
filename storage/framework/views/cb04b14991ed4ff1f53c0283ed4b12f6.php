    ...
    <form method="GET" class="flex flex-wrap gap-3 items-end">
        ...
        <div>
            ...
            <select name="status" class="border-2 border-gray-200 rounded-xl px-4 py-2 text-sm">
                ...
                <option value="revoked" <?php if(request('status') === 'revoked'): echo 'selected'; endif; ?>>ملغاة الاعتماد</option>
                <option value="cancelled" <?php if(request('status') === 'cancelled'): echo 'selected'; endif; ?>>ملغاة من الموظف</option>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tr class="border-t border-gray-100 align-top">
                ...
                <td class="p-3"><?php echo $__env->make('attendances.partials.status-badge', ['label' => $review->statusLabel(), 'color' => $review->isPending() ? 'amber' : (in_array($review->status, ['approved'], true) ? 'green' : 'red')], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                    <?php if($review->deduction_amount): ?>
                    <span class="block text-xs text-red-600 mt-1">خصم: <?php echo e(number_format($review->deduction_amount, 2)); ?></span>
                    <?php endif; ?>
                </td>
                <td class="p-3">
                    ...
                    <?php elseif($review->isApproved()): ?>
                    <form method="POST" action="<?php echo e(route('operations.checkout-reviews.revoke', $review)); ?>" class="flex gap-1 min-w-[200px]">
                        <?php echo csrf_field(); ?>
                    ...
                    <?php elseif($review->review_notes || $review->deduction_reason): ?>
                    <p class="text-xs text-gray-500"><?php echo e($review->review_notes); ?></p>
                    <?php if($review->deduction_reason): ?><p class="text-xs text-red-600 mt-1"><?php echo e($review->deduction_reason); ?></p><?php endif; ?>
                    <?php else: ?>
                    —
                    <?php endif; ?>
                </td>
