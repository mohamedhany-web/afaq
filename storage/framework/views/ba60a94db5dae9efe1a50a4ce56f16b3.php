...
<?php if(($assignableReps ?? collect())->isNotEmpty() || auth()->user()->can('bulkDelete', \App\Models\Client::class)): ?>
<div id="client-bulk-bar" class="hidden mb-4 p-4 rounded-2xl border-2 bg-white shadow-sm font-tajawal" style="border-color: <?php echo e($themeColor); ?>40;">
    <div class="flex flex-col lg:flex-row lg:items-end gap-4">
        ...
        <?php if(($assignableReps ?? collect())->isNotEmpty() && auth()->user()->can('bulkUpdate', \App\Models\Client::class)): ?>
        <form method="POST" action="<?php echo e(route('crm.clients.bulk-transfer')); ?>" class="flex flex-col sm:flex-row gap-2 items-stretch sm:items-end" id="bulk-transfer-form">
            <?php echo csrf_field(); ?>
            <div id="bulk-transfer-ids"></div>
            <div>
                <label class="block text-xs font-bold text-gray-500 mb-1">تحويل إلى مندوب</label>
            ...
            <button type="submit" class="px-4 py-2 rounded-xl text-sm font-semibold text-white" style="background: <?php echo e($themeColor); ?>;"
                    onclick="return confirm('تحويل العملاء المحددين إلى المندوب؟')">تحويل المحدد</button>
        </form>
        <?php endif; ?>
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('bulkDelete', \App\Models\Client::class)): ?>
        <form method="POST" action="<?php echo e(route('crm.clients.bulk-destroy')); ?>" class="flex flex-col gap-2 min-w-[240px]" id="bulk-delete-form"
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\afaq\resources\views\crm\clients\partials\bulk-actions.blade.php ENDPATH**/ ?>