...
<div class="w-full">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p class="text-3xl font-bold text-amber-600"><?php echo e($summary['late_days'] ?? 0); ?></p>
        </div>
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <table class="w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 text-sm text-gray-900"><?php echo e($attendance->date?->format('Y-m-d') ?? $attendance->date); ?></td>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500" dir="ltr"><?php echo e($attendance->scheduled_check_in_at?->format('H:i') ?? '—'); ?></td>
                        <td class="px-6 py-4 text-sm text-gray-600" dir="ltr"><?php echo e($attendance->check_in?->format('H:i') ?? '—'); ?></td>
                        <td class="px-6 py-4 text-sm text-gray-600" dir="ltr"><?php echo e($attendance->check_out?->format('H:i') ?? '—'); ?></td>
                        <td class="px-6 py-4 text-sm <?php echo e(($attendance->late_minutes ?? 0) > 0 ? 'text-amber-700 font-semibold' : 'text-gray-400'); ?>">
                            <?php echo e(($attendance->late_minutes ?? 0) > 0 ? $attendance->late_minutes . ' د' : '—'); ?>

                        </td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900"><?php echo e($attendance->total_hours ?? 0); ?></td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium font-tajawal
                                ...
                                <?php elseif($attendance->status === 'half_day'): ?> bg-orange-100 text-orange-800
                                <?php else: ?> bg-gray-100 text-gray-700 <?php endif; ?>">
                                <?php echo e($statusLabels[$attendance->status] ?? $attendance->status); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\afaq\resources\views\reports\attendance.blade.php ENDPATH**/ ?>