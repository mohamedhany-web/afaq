<?php
    $showLeavesLink = auth()->user() && (
        auth()->user()->can('view-leaves')
        || auth()->user()->usesCrmWorkspace()
        || auth()->user()->usesMarketingWorkspace()
        || auth()->user()->usesOperationsWorkspace()
    );
?>
<?php if($showLeavesLink): ?>
