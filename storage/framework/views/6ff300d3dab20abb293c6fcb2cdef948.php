...
<?php echo $__env->make('crm.partials.page-header', [
    'title' => 'توزيع العملاء على المندوبين',
    'subtitle' => 'ترحيل العملاء الجدد غير المُعيَّنين إلى فريق المبيعات',
...
<div class="grid grid-cols-2 gap-3 mb-6">
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'بانتظار التوزيع', 'value' => $stats['unassigned'], 'accent' => 'amber', 'href' => route('crm.leads.distribution', ['filter' => 'unassigned']) . '#page-data', 'linkLabel' => 'عرض'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'غير متابع +3 أيام', 'value' => $stats['stale'], 'accent' => 'red', 'href' => route('crm.leads.distribution', ['filter' => 'stale']) . '#page-data', 'linkLabel' => 'عرض'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
</div>

<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 font-tajawal" id="page-data">
    <div class="lg:col-span-2 bg-white rounded-2xl border overflow-hidden">
        <div class="px-5 py-4 border-b flex flex-wrap gap-2 items-center justify-between">
            <div class="flex flex-wrap gap-2 items-center">
                <p class="font-bold"><?php echo e(($filter ?? 'unassigned') === 'stale' ? 'عملاء غير متابعين' : 'عملاء بانتظار التوزيع'); ?></p>
                <a href="<?php echo e(route('crm.leads.distribution', ['filter' => 'unassigned'])); ?>#page-data" class="text-xs font-bold px-2 py-1 rounded-lg <?php echo e(($filter ?? 'unassigned') !== 'stale' ? 'text-white' : 'border text-gray-600'); ?>" <?php if(($filter ?? 'unassigned') !== 'stale'): ?> style="background:<?php echo e($themeColor); ?>" <?php endif; ?>>بانتظار التوزيع</a>
                <a href="<?php echo e(route('crm.leads.distribution', ['filter' => 'stale'])); ?>#page-data" class="text-xs font-bold px-2 py-1 rounded-lg <?php echo e(($filter ?? '') === 'stale' ? 'text-white' : 'border text-gray-600'); ?>" <?php if(($filter ?? '') === 'stale'): ?> style="background:<?php echo e($themeColor); ?>" <?php endif; ?>>متأخرون</a>
            </div>
            <?php if(($filter ?? 'unassigned') !== 'stale'): ?>
            <form method="POST" action="<?php echo e(route('crm.leads.auto-distribute')); ?>"><?php echo csrf_field(); ?>
                <button type="submit" class="px-4 py-2 rounded-xl text-white text-xs font-bold" style="background:<?php echo e($themeColor); ?>">توزيع تلقائي</button>
            </form>
            <?php endif; ?>
        </div>
        <form method="GET" class="p-4 border-b">
            <input type="hidden" name="filter" value="<?php echo e($filter ?? 'unassigned'); ?>">
            <input type="search" name="search" value="<?php echo e(request('search')); ?>" placeholder="بحث بالاسم أو الهاتف..." class="w-full border rounded-xl px-4 py-2 text-sm">
        </form>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tr class="border-t">
                    ...
                    <td class="p-3"><a href="<?php echo e($lead->profileUrl()); ?>" class="font-semibold hover:underline" style="color:<?php echo e($themeColor); ?>"><?php echo e($lead->name); ?></a></td>
                    ...
                    <td class="p-3">
                        <?php if(($filter ?? 'unassigned') !== 'stale'): ?>
                        <form method="POST" action="<?php echo e(route('crm.leads.assign', $lead)); ?>" class="flex gap-1"><?php echo csrf_field(); ?>
                ...
                <tr><td colspan="5" class="p-8 text-center text-gray-500">لا يوجد عملاء بانتظار التوزيع</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="space-y-4">
        ...
        <form id="batch-form" method="POST" action="<?php echo e(route('crm.leads.distribute-batch')); ?>" class="bg-white rounded-2xl border p-5"><?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\afaq\resources\views\crm\leads\distribution.blade.php ENDPATH**/ ?>