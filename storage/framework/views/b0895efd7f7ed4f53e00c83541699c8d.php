...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 no-print">
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'تشغيلي', 'value' => ($operatingCashFlow >= 0 ? '+' : '-') . $money(abs($operatingCashFlow)), 'accent' => $operatingCashFlow >= 0 ? 'green' : 'red', 'compact' => true], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'استثماري', 'value' => ($investingCashFlow >= 0 ? '+' : '-') . $money(abs($investingCashFlow)), 'accent' => $investingCashFlow >= 0 ? 'purple' : 'red', 'compact' => true], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'تمويلي', 'value' => ($financingCashFlow >= 0 ? '+' : '-') . $money(abs($financingCashFlow)), 'accent' => $financingCashFlow >= 0 ? 'blue' : 'red', 'compact' => true], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'صافي التغير', 'value' => ($netCashFlow >= 0 ? '+' : '-') . $money(abs($netCashFlow)), 'accent' => $netCashFlow >= 0 ? 'theme' : 'red', 'compact' => true], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
</div>
