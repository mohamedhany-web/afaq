<?php $__env->startSection('page-title', ($operationsOnly ?? false) ? 'مديرو العمليات' : (($marketingOnly ?? false) ? 'موظفو التسويق' : 'موظفو المبيعات')); ?>
...
<div class="grid grid-cols-2 lg:grid-cols-<?php echo e(($operationsOnly ?? false) || ($marketingOnly ?? false) ? '4' : '5'); ?> gap-3 sm:gap-4 mb-6">
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'إجمالي الموظفين', 'value' => $stats['total'], 'accent' => 'theme', 'compact' => true, 'href' => $indexUrl . '#page-data', 'linkLabel' => 'عرض القائمة'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'نشطون', 'value' => $stats['active'], 'accent' => 'green', 'compact' => true, 'href' => route('employees.index', array_filter(['sales_only' => ($salesOnly ?? false) ? 1 : null, 'marketing_only' => ($marketingOnly ?? false) ? 1 : null, 'operations_only' => ($operationsOnly ?? false) ? 1 : null, 'status' => 'active'])) . '#page-data', 'linkLabel' => 'عرض النشطين'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => ($operationsOnly ?? false) ? 'مديرو عمليات' : (($marketingOnly ?? false) ? 'مديرو تسويق' : 'مديرو مبيعات'), 'value' => $stats['managers'], 'accent' => 'purple', 'compact' => true, 'href' => route('employees.index', array_filter(['sales_only' => ($salesOnly ?? false) ? 1 : null, 'marketing_only' => ($marketingOnly ?? false) ? 1 : null, 'operations_only' => ($operationsOnly ?? false) ? 1 : null, 'crm_role' => 'manager'])) . '#page-data', 'linkLabel' => 'عرض المديرين'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php if(!($operationsOnly ?? false) && !($marketingOnly ?? false)): ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => 'قادة فرق', 'value' => $stats['team_leaders'] ?? 0, 'accent' => 'blue', 'compact' => true, 'href' => route('employees.index', array_filter(['sales_only' => ($salesOnly ?? false) ? 1 : null, 'crm_role' => 'team_leader'])) . '#page-data', 'linkLabel' => 'عرض القادة'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php endif; ?>
    <?php if(!($operationsOnly ?? false)): ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => ($marketingOnly ?? false) ? 'موظفو تسويق' : 'مندوبو مبيعات', 'value' => $stats['agents'], 'accent' => 'amber', 'compact' => true, 'href' => route('employees.index', array_filter(['sales_only' => ($salesOnly ?? false) ? 1 : null, 'marketing_only' => ($marketingOnly ?? false) ? 1 : null, 'crm_role' => 'agent'])) . '#page-data', 'linkLabel' => 'عرض المندوبين'], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php endif; ?>
</div>
...
<div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-4 sm:p-5 mb-6">
    <form method="GET" class="flex flex-col lg:flex-row gap-3 lg:items-end">
        ...
        <?php if($operationsOnly ?? false): ?><input type="hidden" name="operations_only" value="1"><?php endif; ?>
        <div class="flex-1">
        ...
        <div class="flex gap-2">
            ...
            <a href="<?php echo e(route('employees.index', array_filter(['sales_only' => ($salesOnly ?? false) ? 1 : null, 'marketing_only' => ($marketingOnly ?? false) ? 1 : null, 'operations_only' => ($operationsOnly ?? false) ? 1 : null]))); ?>" class="px-5 py-2.5 rounded-xl border-2 border-gray-200 text-gray-600 text-sm font-semibold hover:bg-gray-50 font-tajawal">مسح</a>
            <?php endif; ?>
        </div>
    </form>
</div>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td class="p-4">
                        <div class="flex flex-wrap gap-1.5">
                            <a href="<?php echo e(route('employees.dossier', array_merge(['employee' => $employee], $query))); ?>"
                               class="px-2.5 py-1.5 rounded-lg text-xs font-semibold border font-tajawal"
                               style="border-color:<?php echo e($themeColor); ?>40;color:<?php echo e($themeColor); ?>">الملف</a>
                ...
                <tr>
                    <td colspan="7" class="p-12 text-center">
                        ...
                        <?php if($canCreate): ?>
                        <a href="<?php echo e(route('employees.create', array_filter(['sales_only' => ($salesOnly ?? false) ? 1 : null, 'marketing_only' => ($marketingOnly ?? false) ? 1 : null, 'operations_only' => ($operationsOnly ?? false) ? 1 : null]))); ?>"
