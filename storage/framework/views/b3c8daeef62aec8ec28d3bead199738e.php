...
<div class="bg-white rounded-2xl border border-gray-200 overflow-hidden font-tajawal ui-compact-hidden operations-kpi-card text-start">
    <div class="px-5 py-4 border-b flex items-center justify-between gap-3" style="background: linear-gradient(135deg, <?php echo e($themeColor); ?>08 0%, transparent 100%);">
        <div>
            ...
            <p class="text-xs text-gray-500"><?php echo e(__('operations.kpi.total_score')); ?>: <?php echo e(number_format($group['score'], 1)); ?>%</p>
        </div>
        <?php if(!empty($link)): ?>
        <a href="<?php echo e($link); ?>" class="text-xs font-bold px-3 py-1.5 rounded-lg border hover:bg-gray-50" style="color:<?php echo e($themeColor); ?>"><?php echo e(__('operations.actions.view_details')); ?></a>
        <?php endif; ?>
    </div>
    <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <?php $__currentLoopData = $primaryItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php echo $__env->make('operations.partials.kpi-item', ['item' => $item, 'itemLinks' => $itemLinks, 'themeColor' => $themeColor], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>

    <?php if($timingItems->isNotEmpty()): ?>
    <div class="px-4 pb-4" x-data="{ open: false }">
        <button type="button" @click="open = !open"
            ...
            <span class="flex items-center gap-2 text-sm font-bold text-gray-700">
                <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-white shrink-0" style="background:<?php echo e($themeColor); ?>">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                </span>
                <?php echo e(__('operations.kpi.timing_toggle')); ?>

                <span class="text-xs font-normal text-gray-500">(<?php echo e($timingItems->count()); ?>)</span>
            </span>
        ...
        <div x-show="open" x-cloak class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <?php $__currentLoopData = $timingItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php echo $__env->make('operations.partials.kpi-item', ['item' => $item, 'itemLinks' => $itemLinks, 'themeColor' => $themeColor], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
    </div>
