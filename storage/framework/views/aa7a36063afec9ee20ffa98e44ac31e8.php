...
<?php if($pins->isNotEmpty() && $primary): ?>
<div class="<?php echo e($embedded ? 'w-full' : 'bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden w-full'); ?>" id="<?php echo e($panelId); ?>">
    <?php if(!$embedded): ?>
    ...
    <?php endif; ?>
            ...
            <select class="google-pin-select border-2 border-gray-200 rounded-xl px-3 py-2 text-sm font-tajawal max-w-xs"
                    data-panel="<?php echo e($panelId); ?>">
                <?php $__currentLoopData = $pins; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $pin): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($i); ?>"
                            data-lat="<?php echo e($pin->latitude); ?>"
                            data-lng="<?php echo e($pin->longitude); ?>"
                            data-title="<?php echo e($pin->title); ?>">
    ...
    <div class="border-b border-gray-100 px-4 sm:px-5 pt-3 flex flex-wrap gap-2 google-map-tabs" data-panel="<?php echo e($panelId); ?>">
        <?php if($hasEmbed): ?>
        ...
        <?php endif; ?>
        <button type="button" data-tab="links" class="gmap-tab px-4 py-2 rounded-t-xl text-xs font-bold font-tajawal border-2 <?php echo e($hasEmbed ? 'border-transparent text-gray-500' : 'border-b-0 border-gray-200 bg-white text-gray-700'); ?>">روابط خارجية</button>
    </div>

    <div class="p-4 sm:p-5">
        <?php if($hasEmbed): ?>
        <div class="gmap-pane hidden" data-pane="satellite" data-panel="<?php echo e($panelId); ?>">
            <iframe class="gmap-iframe w-full rounded-xl border border-gray-200" style="height:380px;border:0;" loading="lazy" allowfullscreen
                    ...
                    src="<?php echo e(GoogleMapsHelper::satelliteEmbedUrl($primary->latitude, $primary->longitude)); ?>"></iframe>
        </div>
        <div class="gmap-pane hidden" data-pane="streetview" data-panel="<?php echo e($panelId); ?>">
            <iframe class="gmap-iframe w-full rounded-xl border border-gray-200" style="height:380px;border:0;" loading="lazy" allowfullscreen
                    referrerpolicy="no-referrer-when-downgrade"
        ...
        <div class="gmap-pane hidden" data-pane="place" data-panel="<?php echo e($panelId); ?>">
            <iframe class="gmap-iframe w-full rounded-xl border border-gray-200" style="height:380px;border:0;" loading="lazy" allowfullscreen
                    referrerpolicy="no-referrer-when-downgrade"
                    src="<?php echo e(GoogleMapsHelper::placeEmbedUrl($primary->latitude, $primary->longitude, $primary->title)); ?>"></iframe>
        </div>
        <?php else: ?>
        ...
        <div class="gmap-pane <?php echo e($hasEmbed ? 'hidden' : ''); ?>" data-pane="links" data-panel="<?php echo e($panelId); ?>">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <a href="<?php echo e(GoogleMapsHelper::mapsUrl($primary->latitude, $primary->longitude, $primary->title)); ?>"
                ...
                <a href="<?php echo e(GoogleMapsHelper::streetViewUrl($primary->latitude, $primary->longitude)); ?>"
                   target="_blank" rel="noopener noreferrer"
                   class="gmap-ext-link flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 hover:border-gray-300 hover:bg-gray-50 transition font-tajawal"
                ...
                <a href="<?php echo e(GoogleMapsHelper::satelliteUrl($primary->latitude, $primary->longitude)); ?>"
                   target="_blank" rel="noopener noreferrer"
                   class="gmap-ext-link flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 hover:border-gray-300 hover:bg-gray-50 transition font-tajawal"
                   data-link="satellite">
                ...
                <a href="<?php echo e(GoogleMapsHelper::directionsUrl($primary->latitude, $primary->longitude)); ?>"
                   target="_blank" rel="noopener noreferrer"
                   class="gmap-ext-link flex items-center gap-3 p-4 rounded-xl border-2 border-gray-200 hover:border-gray-300 hover:bg-gray-50 transition font-tajawal"
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                <button type="button" class="gmap-copy-link px-4 py-2 rounded-xl text-xs font-semibold font-tajawal text-white"
                        ...
                        data-url="<?php echo e(GoogleMapsHelper::mapsUrl($primary->latitude, $primary->longitude, $primary->title)); ?>">
                    نسخ رابط المشاركة
                </button>
                <button type="button" class="gmap-copy-link px-4 py-2 rounded-xl text-xs font-semibold font-tajawal border-2 border-gray-200 text-gray-700"
                        data-url="<?php echo e(GoogleMapsHelper::streetViewUrl($primary->latitude, $primary->longitude)); ?>">
...
document.addEventListener('DOMContentLoaded', function () {
    const panelId = <?php echo json_encode($panelId, 15, 512) ?>;
    const hasEmbed = <?php echo json_encode($hasEmbed, 15, 512) ?>;
    const apiKey = <?php echo json_encode(GoogleMapsHelper::apiKey(), 15, 512) ?>;
    const root = document.getElementById(panelId);
    if (!root) return;

    const pins = <?php echo json_encode($pins->map(fn ($p) => [
        'title' => $p->title, 'lat' => (float) $p->latitude, 'lng' => (float) $p->longitude) ?>;
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\afaq\resources\views\projects\partials\google-maps-panel.blade.php ENDPATH**/ ?>