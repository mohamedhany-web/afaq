...
<?php $__env->startSection('page-title', 'مراجعة طلب عميل'); ?>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 font-tajawal">
    <div class="lg:col-span-2 bg-white rounded-2xl shadow-lg border border-gray-200 p-5 sm:p-6 space-y-4">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            <div><dt class="text-xs text-gray-500">العميل الحالي</dt><dd><a href="<?php echo e($request->client->profileUrl()); ?>" class="font-bold" style="color:<?php echo e($themeColor); ?>"><?php echo e($request->client->name); ?></a></dd></div>
            <?php endif; ?>
        </dl>

        <?php if($request->action !== 'delete' && !empty($clientData)): ?>
        <div class="border-t pt-4 space-y-2 text-sm">
            <p><span class="text-gray-500">الاسم:</span> <strong><?php echo e($clientData['name'] ?? '—'); ?></strong></p>
            <p><span class="text-gray-500">الهاتف:</span> <?php echo e($clientData['phone'] ?? '—'); ?></p>
            <p><span class="text-gray-500">البريد:</span> <?php echo e($clientData['email'] ?? '—'); ?></p>
            <p><span class="text-gray-500">الحالة:</span> <?php echo e($clientData['status'] ?? '—'); ?></p>
            <?php if(!empty($clientData['notes'])): ?>
            <p class="text-gray-700 whitespace-pre-wrap"><?php echo e($clientData['notes']); ?></p>
            <?php endif; ?>
        </div>
        <?php elseif($request->action === 'delete'): ?>
        <div class="text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl p-4 space-y-2">
            <p>طلب حذف العميل: <strong><?php echo e($payload['client_name'] ?? $request->client?->name); ?></strong></p>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-5 sm:p-6">
        ...
        <form action="<?php echo e(route('crm.clients.approvals.approve', $request)); ?>" method="POST" class="mb-4">
            <?php echo csrf_field(); ?>
        ...
        <form action="<?php echo e(route('crm.clients.approvals.reject', $request)); ?>" method="POST">
            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\afaq\resources\views\crm\clients\approvals\show.blade.php ENDPATH**/ ?>