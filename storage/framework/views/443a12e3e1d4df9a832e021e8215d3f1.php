...
<?php $__env->startSection('page-title', __('operations.dashboard_title')); ?>
...
<div class="operations-locale-surface <?php echo e($isLtr ? 'text-start' : 'text-right'); ?>" <?php if($isLtr): ?> dir="ltr" <?php endif; ?>>
...
<div class="flex flex-wrap items-center gap-3 mb-6 font-tajawal">
    <?php echo $__env->make('partials.ui-compact-toggle', ['themeColor' => $themeColor, 'labelOn' => __('operations.ui.compact_on'), 'labelOff' => __('operations.ui.compact_off')], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
<?php if(!empty($selectedSalesRep)): ?>
<div class="mb-6 p-4 rounded-2xl border-2 font-tajawal flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
     style="border-color: <?php echo e($themeColor); ?>40; background: <?php echo e($themeColor); ?>08;">
    <div>
        <p class="text-xs font-bold text-gray-500"><?php echo e(__('operations.dashboard.filtered_by_rep')); ?></p>
        <p class="text-lg font-extrabold" style="color: <?php echo e($themeColor); ?>;"><?php echo e($selectedSalesRep->name); ?></p>
        <p class="text-xs text-gray-600 mt-1"><?php echo e(__('operations.dashboard.filtered_hint')); ?></p>
    </div>
    <div class="flex flex-wrap gap-2">
        <a href="<?php echo e(route('operations.clients.index', ['view' => 'data', 'sales_rep' => $selectedSalesRep->id])); ?>"
           ...
           style="background: <?php echo e($themeColor); ?>;">
            <?php echo e(__('operations.dashboard.view_rep_clients')); ?>

        </a>
        <a href="<?php echo e(route('operations.reps.show', $selectedSalesRep)); ?>"
           ...
           style="border-color: <?php echo e($themeColor); ?>40; color: <?php echo e($themeColor); ?>;">
            <?php echo e(__('operations.actions.open_rep_workspace')); ?>

        </a>
        <a href="<?php echo e(route('operations.dashboard')); ?>"
           class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold bg-white border border-gray-200 text-gray-600 hover:bg-gray-50">
            <?php echo e(__('operations.dashboard.clear_filter')); ?>
...
<div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 mb-6" id="page-data">
    <?php $__currentLoopData = $workspaceSections; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $section): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6 ui-compact-hidden">
    <?php echo $__env->make('crm.partials.stat-card', ['label' => __('operations.stats.pending_distribution'), 'value' => $stats['unassigned_leads'], 'accent' => 'amber', 'href' => route('operations.clients.index', ['view' => 'distribution']) . '#page-data', 'linkLabel' => __('operations.clients.tab_distribution')], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => __('operations.stats.absence_reviews'), 'value' => $stats['pending_absence_reviews'], 'accent' => 'red', 'href' => ($absenceReviewsLink ?? route('operations.attendance-reviews.index', ['status' => 'pending'])) . '#page-data', 'linkLabel' => __('operations.actions.view_details')], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => __('operations.stats.checkout_approvals'), 'value' => $stats['pending_checkout_reviews'], 'accent' => 'purple', 'href' => route('operations.checkout-reviews.index') . '#page-data', 'linkLabel' => __('operations.actions.view_details')], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <?php echo $__env->make('crm.partials.stat-card', ['label' => __('operations.stats.active_projects'), 'value' => $stats['active_projects'], 'accent' => 'theme', 'href' => route('operations.inventory.index') . '#page-data', 'linkLabel' => __('operations.actions.view_details')], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6 font-tajawal">
    <div class="bg-white rounded-2xl border p-5 ui-compact-hidden text-start">
        <p class="text-xs text-gray-500 mb-1"><?php echo e(__('operations.stats.compensation_kpis')); ?> — <?php echo e($period->label); ?></p>
        <p class="text-3xl font-extrabold" style="color:<?php echo e($themeColor); ?>"><?php echo e(number_format($kpi['total_score'] ?? 0, 1)); ?>%</p>
        <a href="<?php echo e(route('crm.compensation.dashboard')); ?>" class="inline-flex items-center gap-1 text-xs font-bold mt-3 hover:underline" style="color:<?php echo e($themeColor); ?>"><?php echo e(__('operations.stats.compensation_details')); ?> <?php echo e($detailArrow); ?></a>
    </div>
    <div class="lg:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <a href="<?php echo e(route($action['route'], $action['params'] ?? [])); ?>" class="flex flex-col items-center gap-2 p-4 rounded-2xl border bg-white hover:shadow-md transition-shadow text-center group">
