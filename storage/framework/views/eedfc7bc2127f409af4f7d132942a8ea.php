...
<form action="<?php echo e(route('crm.projects.store')); ?>" method="POST" class="w-full space-y-6">
    <?php echo csrf_field(); ?>
    <?php echo $__env->make('projects.partials.form', ['project' => $project, 'users' => $users, 'developers' => $developers ?? collect(), 'themeColor' => $themeColor], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 pb-6">
        ...
        <button type="submit" class="inline-flex items-center justify-center px-8 py-3 rounded-xl text-white font-semibold text-sm shadow-md font-tajawal"
                style="background: linear-gradient(135deg, <?php echo e($themeColor); ?> 0%, <?php echo e($themeColor); ?>dd 100%);"><?php echo e(($requiresApproval ?? false) ? 'إرسال للموافقة' : 'حفظ المشروع'); ?></button>
    </div>
</form>
